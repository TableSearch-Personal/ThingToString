...
function _captureUISnapshot() {
    const snap = {
        ...
        diagLog             : document.getElementById("diagLog").checked,
        fileOrderSelect     : document.getElementById("fileOrderSelect").value,
        useStreamSaver      : document.getElementById("useStreamSaver").checked,
    };
    return snap;
}
...
function _restoreUISnapshot(snap) {
    ...
    document.getElementById("diagLog").checked                = snap.diagLog;
    document.getElementById("fileOrderSelect").value          = snap.fileOrderSelect;
    document.getElementById("useStreamSaver").checked         = snap.useStreamSaver;
...
function exportLog() {
    ...
    const lines = [];
    const now = new Date();
    lines.push(`ThingToString v${APP_VERSION} — Log export`);
    ...
    const a    = Object.assign(document.createElement("a"),
        { href: url, download: `tsf-log-${ts}.txt` });
    document.body.appendChild(a);
    a.click();
    setTimeout(() => { URL.revokeObjectURL(url); a.remove(); }, 1000);
    flash("Log exported", "success", 2000);
}

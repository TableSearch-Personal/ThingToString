...
async function decryptFileV18(file, password) {
    ...
    if (chunkCount === 0)
        throw new TSFError("corrupt", "Chunk count is zero — file is malformed");
    ...
    // BUG FIX: fullAAD must match exactly what encrypt.js used as `aad` for
    // the chunk encryption loop.  encrypt.js builds:
    //   aad = fixedHeader + encMetaBlob   (when metaEncrypted)
    //   aad = fixedHeader                 (otherwise)
    // where encMetaBlob = [metaIV (12 B)] + [encMetaLen (4 B)] + [encMeta].
    //
    // The old code set fullAAD = fullFixedBuf, which only covered the fixed
    // header region and excluded the encMetaBlob bytes entirely.  This made
    // every chunk's AAD mismatch the one used during encryption, causing
    // AES-GCM authentication to fail on chunk 0 with "wrong password".
    //
    // Fix: read the encMetaBlob bytes from the file (they sit immediately
    // after the fixed header, before the first chunk) and append them to
    // fullFixedBuf to reproduce the exact aad that encrypt.js produced.
    let fullAAD;
    if (metaEncrypted) {
        // encMetaBlob spans from totalFixedLen up to the current fileOffset
        // (which was advanced past metaIV + encMetaLen + encMeta above).
        const encMetaBlob = new Uint8Array(
            await file.slice(totalFixedLen, fileOffset).arrayBuffer());
        fullAAD = concatArrays([fullFixedBuf, encMetaBlob]);
    } else {
        fullAAD = fullFixedBuf;
    }
    ...
    for (let i = 0; i < chunkCount; i++) {
        ...
        if (cipherLen === 0 || cipherLen > chunkSize + 512)
            throw new TSFError("corrupt",
                `Chunk ${i}: implausible ciphertext length ${cipherLen} — header corrupt or file truncated`);
        fileOffset += 4;
...
async function decryptFileV17(file, password) {
    ...
    if (chunkCount > MAX_CHUNK_COUNT)
        throw new TSFError("corrupt", `Chunk count out of range: ${chunkCount}`);
    if (chunkCount === 0)
        throw new TSFError("corrupt", "V17: Chunk count is zero — file is malformed");
    if (metaLen > MAX_META_BYTES)
        throw new TSFError("corrupt", `Metadata length out of range: ${metaLen}`);
    if (compByte !== COMPRESSION_STANDARD && compByte !== COMPRESSION_AGGRESSIVE &&
        compByte !== COMPRESSION_NONE)
...
async function decryptFileV15(file, password) {
    ...
    if (chunkCount === 0)
        throw new TSFError("corrupt", "V15: Chunk count is zero — file is malformed");
    if (metaLen > MAX_META_BYTES)
    ...
    for (let i = 0; i < chunkCount; i++) {
        ...
        if (cipherLen === 0 || cipherLen > chunkSize + 512)
            throw new TSFError("corrupt",
                `V15 Chunk ${i}: implausible ciphertext length ${cipherLen} — header corrupt or file truncated`);
        fileOffset += 4;

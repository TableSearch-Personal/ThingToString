...
async function deriveKey(password, salt, parallelism, argon2Params = {}) {
    ...
    let result;
    try {
        log("  [diag] calling argon2.hash()…", "info");
        ...
        log(`  [diag] argon2.hash() resolved OK in ${_kdfMs}ms`, "info");
    } catch (e) {
        ...
        log(`  [diag] argon2.hash() threw: ${msg}`, "error");
    ...
    if (settings.useStreamSaver && typeof streamSaver !== "undefined") {
        ...
        if (!isSecureContext) {
            ...
            try {
                ...
                // more than 30s, the SW has likely crashed. Surface as an error
